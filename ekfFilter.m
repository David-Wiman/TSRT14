...
% 
% meas: struct with fields:mean_acc
%       t: timestamp (1 x T)
...
% Measurements not available are marked with NaNs.
  ...
  while server.status() && ~ownView.quit
    ...
    if ~any(isnan(acc))  % Acc measurements are available.
        ...
        % Probaly left-oriented coordinate system or sensors in phone not
        % standard
        if norm(acc) < 10.5 && norm(acc) > 9.5
    ...
    if ~any(isnan(mag))  % Mag measurements are available.
        m0 = 1e-6*[0, sqrt(  calMag.m(1)^2 +  calMag.m(2)^2 ),  calMag.m(3)]'; % nominal magnetic field
